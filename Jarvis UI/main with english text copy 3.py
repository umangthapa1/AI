...
class Ui_Dialog(QtWidgets.QMainWindow):
    def setupUi(self, Dialog):
        ...
        self.label_listening.setPixmap(QtGui.QPixmap("listening.gif"))
        ...
        self.label_speaking.setPixmap(QtGui.QPixmap("speaking.gif"))
        ...
        self.label_recognizing.setPixmap(QtGui.QPixmap("recognizing.gif"))
        ...
        self.label_searching.setPixmap(QtGui.QPixmap("searching.gif"))
        ...
        self.label_2.setPixmap(QtGui.QPixmap("terminal.png"))
        ...
        self.label_4.setPixmap(QtGui.QPixmap("discord.png"))
...
genai.configure(api_key='AIzaSyBdmmyIDmylJY2wlPxIY77Dth7kvNSl2YE')
...
dataset=[
    ...
    },
    {
        ...
        "respo": [
            "Sure, if you need any other assistance then I am still online."
        ]
    }
]
...
def handle_command(query, outputterminalBox):
    ...
    if any(word in query for word in ['who', 'what', 'where', 'when', 'why', 'how', 'tell me about', 'can you give me the code']):
    ...
    elif 'do i have any new messages' in query:
        speak("Sure! Sir let me check.")
        outputterminalBox.appendPlainText("let me check")
        if 'instagram' in query:
            ...
            speak("We are having some technical issues. Please try again later.")
...
def clean_up_url(texts):
    cleaned_url = texts.replace('dot', '.').replace('slash', '/').replace(' ', '')
    return cleaned_url

class ListenThread(QtCore.QThread):
    def __init__(self, app, dialog, ui):
        super().__init__()
        self.running = True
    ...
    def run(self):
        self.ui.updateui("neutral")
        wake_up_phrase = ("hey assistant", "wake up assistant")
        minimize_phrase = "take some rest"
    ...
    def minimize_to_tray(self):
        global tray_icon
        tray_icon = QSystemTrayIcon(QtGui.QIcon("logo.png"), parent=self.app)
        tray_icon.show()
        self.dialog.hide()
...
if __name__ == "__main__":
    ...
    ui.movie4= QtGui.QMovie("listening.gif")
    ui.label_listening.setMovie(ui.movie4)
    ui.movie4.start()

    ui.movie5= QtGui.QMovie("speaking.gif")
    ui.label_speaking.setMovie(ui.movie5)
    ui.movie5.start()

    ui.movie6= QtGui.QMovie("recognizing.gif")
    ui.label_recognizing.setMovie(ui.movie6)
    ui.movie6.start()

    ui.movie7= QtGui.QMovie("searching.gif")
    ...
    listen_thread = ListenThread(app, Dialog, ui)
    listen_thread.start()
